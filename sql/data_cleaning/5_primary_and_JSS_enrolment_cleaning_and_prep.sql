/*
  ...
  Purpose: Data cleaning and preparation of primary and junior secondary school enrolment data
  Dataset: raw_data/20260522_KNBS_Primary and Junior School Enrolment by Class and Gender.csv
*/

-- Prep work: Cleaning up column naming and immediately identifiable errors in the data table
WITH cleaned_enrolment_data AS (
	SELECT -- formatting field names in snake_case
	  `Frequency _A_` AS frequency,
	  `Education level __T_` AS education_level,
	  `Grade _ year of study __T_` AS grade_or_class,
	  `Sex _M_` AS gender,
	  `Unit of measure _NUMBER_` AS uom,
	  `Time period _2015_` AS year,
	  Observation AS original_value,
	  -- Converting Observation column (values) from strings to floats, and fixing error discovered in some values where numbers erroneously had more than 1 period
	  CASE
	  WHEN Observation LIKE "%.%.%"
	    THEN CAST(REGEXP_REPLACE(Observation, r'(.*)\.(.*)\.(.*)', r'\1\2.\3') AS FLOAT64)
	    ELSE CAST(Observation AS FLOAT64)
...
grades_missing_year_data AS(
  ...
  GROUP BY 
    ...
    grade_or_class
  HAVING 
    no_years_reported_on < (SELECT COUNT (DISTINCT year) FROM cleaned_enrolment_data)
...
records_missing_gender_data AS(
  ...
  GROUP BY 
    ...
    year
  HAVING 
    no_gender_metrics_reported_on < (SELECT COUNT (DISTINCT gender) FROM cleaned_enrolment_data)
...
duplicates_check AS (
  SELECT
    ARRAY_TO_STRING([CAST(year AS string), education_level, grade_or_class, gender, CAST(cleaned_value AS string)],"_") AS unique_identifier,
...
out_of_bounds_values AS (
  SELECT
    COUNTIF(cleaned_value < 0) AS negative_values,
    COUNTIF(TRUNC(cleaned_value*1000)<>(cleaned_value*1000)) AS non_integer_values -- Multiplied by 1000 as this is indicated as the multiplier for all values
  FROM
    cleaned_enrolment_data
),
...
contradictions_check AS (
  SELECT
    ARRAY_TO_STRING([CAST(year AS string), education_level, grade_or_class, gender],"_") AS value_identifier,
...
enrolment_per_grade_per_year AS (
  SELECT
    ...
    education_level,
    grade_or_class,
    SUM(CASE WHEN gender = "Male" THEN cleaned_value*1000 ELSE 0 END) AS males_enrolled,
    SUM(CASE WHEN gender = "Female" THEN cleaned_value*1000 ELSE 0 END) AS females_enrolled,
    SUM(CASE WHEN gender = "Total" THEN cleaned_value*1000 ELSE 0 END) AS reported_total_enrolment,
    SUM(CASE WHEN gender = "Male" THEN cleaned_value*1000 ELSE 0 END) + SUM(CASE WHEN gender = "Female" THEN cleaned_value*1000 ELSE 0 END) AS calculated_total_enrolment
  FROM
    cleaned_enrolment_data
  GROUP BY 
    year,
...
-- This table checks for cases of enroled students in the 844 only classes (standard 7 and 8) after enrolment in these classes should have concluded
likely_erronous_844_enrolments AS (
  SELECT
    COUNT(*) AS likely_erroneous_844_enrolment_data
  FROM
    cleaned_enrolment_data
  WHERE
    ((grade_or_class = "Standard 7" AND year > 2022) OR (grade_or_class = "Standard 8" AND year > 2023)) AND cleaned_value IS NOT null
  
),

-- This table checks for cases of enroled students in the CBE only classes (grade 7 and 8) prior to when enrolment in these classes should have commenced
likely_erronous_CBE_enrolments AS (
  SELECT
    COUNT(*) AS likely_erroneous_CBE_enrolment_data
  FROM
    cleaned_enrolment_data
  WHERE
    ((grade_or_class = "Grade 7" AND year < 2023) OR (grade_or_class = "Grade 8" AND year < 2024)) AND cleaned_value IS NOT null
),
...
FROM
  data_completeness, out_of_bounds_values, inconsistent_data, likely_erronous_844_enrolments, likely_erronous_CBE_enrolments, remarks
