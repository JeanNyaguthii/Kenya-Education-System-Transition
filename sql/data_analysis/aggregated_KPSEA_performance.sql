/*
  ...
  Purpose: Aggregate KPSEA performance data to assess year-over-year trends
  Data Sources:
  ...
  Methodology: The raw datasets do not provide aggregated performance across all subjects. This script calculates the weighted average performance by weighting subject-level scores against the number of candidates per subject. 
...
-- Using 2023 data to calculate subject-specific candidate ratios 
...
-- Calculating the aggregated performance year on year, by calculating weighted averages (subject-level scores weighed against the candidate ratio for the subject)

aggregate_performance_by_level_by_year AS (
  SELECT
    performance_level,
		year,
		SUM(percent_of_students * ratio_of_all_candidates_who_took_subject) / SUM(ratio_of_all_candidates_who_took_subject) AS weighted_average_across_all_subjects
	FROM
...
SELECT
	*
FROM
	aggregate_performance_by_level_by_year
